-- Allow a manager-uploaded organization mark. Fathers see it next to the
-- group name at the top of every page. Does not restore cover-photo slots.
...
alter table public.organization_photos
  ...
  check (slot ~ '^(home_hero|home_profile|org_logo|training_[a-z0-9]+(-[a-z0-9]+)*)$');

comment on column public.organization_photos.slot is
  'home_hero, home_profile, org_logo, or training_{slug}.';
...
as $$
  select
    ...
    and internal.org_photo_slot_name(object_name)
      ~ '^(home_hero|home_profile|org_logo|training_[a-z0-9]+(-[a-z0-9]+)*)$'
    and (
