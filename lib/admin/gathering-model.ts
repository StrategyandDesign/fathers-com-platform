...
export type GatheringHomePreview = {
  fathersSharing: string;
  leadersSharing: string;
  reviewersSharing: string;
  sessionsCompleted: number | null;
  assignments: number | null;
  certificatesIssued: number | null;
};

export function gatheringHomePreview(gathering: AdminGathering): GatheringHomePreview {
  return {
    fathersSharing: `${gathering.fathers.optedIn} of ${gathering.fathers.eligible}`,
    leadersSharing: `${gathering.managers.optedIn} of ${gathering.managers.eligible}`,
    reviewersSharing: `${gathering.reviewers.optedIn} of ${gathering.reviewers.eligible}`,
    sessionsCompleted: gathering.fathers.sessionsCompleted,
    assignments: gathering.managers.assignments,
    certificatesIssued: gathering.managers.certificatesIssued,
  };
}

export function parseAdminGathering(data: Record<string, unknown>): AdminGathering {
  const fathersRaw = (data.fathers ?? {}) as RawSlice & Record<string, unknown>;
  const managersRaw = (data.managers ?? {}) as RawSlice & Record<string, unknown>;
  const reviewersRaw = (data.reviewers ?? {}) as RawSlice & Record<string, unknown>;
  ...
  return {
    unavailable: false,
    minCohort: asInt(data.min_cohort, GATHERING_MIN_COHORT),
    fathers: {
      ...fathers,
