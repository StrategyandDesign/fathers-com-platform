...
export const PROFILE_QUESTIONS: ProfileQuestion[] = Array.from(
  { length: PROFILE_QUESTION_COUNT },
  (_, index) => {
    ...
    const qualifier =
      QUALIFIERS[Math.floor(index / (PROFILE_THEMES.length * stems.length)) % QUALIFIERS.length];
...
export function parseAnswers(value: unknown): Record<string, number> {
...
export function firstUnanswered(answers: Record<string, number>) {
  const missing = PROFILE_QUESTIONS.find((question) => answers[String(question.id)] == null);
  return missing?.id ?? PROFILE_QUESTION_COUNT;
}

export function hasAllAnswers(answers: Record<string, number>) {
