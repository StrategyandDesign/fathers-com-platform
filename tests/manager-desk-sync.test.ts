...
describe("leader desk live loop", () => {
  it("polls a manager-only stamp and refreshes every Leader desk", () => {
    ...
    const helpers = readRepo("lib/manager/desk-sync.ts");
    const sync = readRepo("lib/manager/desk-sync-data.ts");
    assert.match(photos, /recordOrganizationActivity/);
    assert.match(assessments, /loadOrgManagerIds/);
    assert.match(route, /desk-sync-data/);
    ...
    assert.doesNotMatch(helpers, /supabase\/server/);
    assert.doesNotMatch(helpers, /next\/headers/);
    assert.doesNotMatch(client, /desk-sync-data/);
