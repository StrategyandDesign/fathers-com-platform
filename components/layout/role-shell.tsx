...
export async function RoleShell({
  ...
  const keystoneRoom = isKeystoneRoomPath(pathname);
  const funnel =
    startFlow ||
    (role === "father" && onboardingActive) ||
    (role === "manager" && onboardingActive);
  const hideChrome = funnel || keystoneRoom;
  const fatherMobile = role === "father" && !hideChrome;
  const managerMobileNav = role === "manager" && !hideChrome;
    ...
    <div className="min-h-svh bg-background">
      ...
      <main
        className={cn(
          ...
          hideChrome ? "print:pt-0" : "lg:ps-[5.5rem] print:pt-0 print:ps-0",
          fatherMobile
      ...
      >
        <div
          className={
            keystoneRoom
              ? "mx-auto flex min-h-svh w-full min-w-0 max-w-2xl flex-col justify-center px-4 py-6"
