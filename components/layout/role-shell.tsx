...
export function RoleShell({
  ...
  return (
    <div className="min-h-svh bg-background">
      <header className="fixed inset-x-0 top-0 z-30 flex h-[calc(3.5rem+env(safe-area-inset-top))] items-center justify-between border-b border-border bg-background/90 px-3 pt-[env(safe-area-inset-top)] backdrop-blur-md lg:px-5">
      ...
      <aside className="fixed bottom-0 left-0 top-[calc(3.5rem+env(safe-area-inset-top))] z-20 hidden w-[5.5rem] flex-col border-r border-border bg-sidebar lg:flex">
        <AppNav role={role} layout="side" />
      </aside>

      {fatherMobile ? (
        <div className="fixed inset-x-0 bottom-0 z-30 border-t border-border bg-background/95 pb-[env(safe-area-inset-bottom)] backdrop-blur-md lg:hidden">
