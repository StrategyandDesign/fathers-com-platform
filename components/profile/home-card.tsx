...
export async function ProfileHomeCard({
  profile,
  draft,
  coverSrc,
  neverStarted = false,
      ...
      >
        <div className="flex min-h-56 flex-1 flex-col p-4 sm:min-h-64 sm:p-5">
          ...
          {profile ? (
            ...
            <p className="mt-3 text-sm text-white/65">{t("father.home.profileLater")}</p>
          ) : draft ? (
            <>
              ...
              <p className="mt-2 text-sm text-white/65">{t("father.home.profileReminder")}</p>
              {hideActions ? null : (
                <div className="mt-auto pt-5">
